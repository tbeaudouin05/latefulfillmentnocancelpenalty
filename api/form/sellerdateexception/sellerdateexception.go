...
import (
	...
	"github.com/thomas-bamilo/vs/latefulfillmentnocancelpenalty/row/sellerdateexception"
...
func Start(c *gin.Context) {
	...
	// only pass form as sellerDateException since we only want a blank form at start
	sellerDateException := &sellerdateexception.SellerDateException{}

	// render the web page itself given the html template and the sellerDateException
	sellerDateException.Render(c, `template/form/sellerdateexception/sellerdateexception.html`)
}
...
func AnswerForm(c *gin.Context) {
	...
	// LoadToDb uploads the purchase request form user inputs (= sellerDateException) to database
	dbBaa := connectdb.ConnectToBaa()
	err = baainteract.LoadSellerDateExceptionToDb(lfncPenaltyException, dbBaa)
	handleErr(c, err)

	// if everything goes well, redirect user to confirmation web page
	http.Redirect(c.Writer, r, `/form/sellerdateexceptionconfirmation`, http.StatusSeeOther)
}
...
func ConfirmForm(c *gin.Context) {
	...
	sellerDateException.Render(c, `template/form/sellerdateexception/sellerdateexceptionconfirmation.html`)
}
